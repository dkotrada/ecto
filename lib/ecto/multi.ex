defmodule Ecto.Multi do
  ...
  @type fun(result) :: ((changes) -> {:ok, result} | {:error, any})
  ...
  defp operation_fun(:insert_or_update, opts, fun) do
    fn repo, changes ->
      case fun.(changes) do
        {:ok, %Changeset{data: %{__meta__: %{state: :loaded}}} = changeset} ->
  ...
  defp operation_fun(operation, opts, fun) do
    fn repo, changes ->
      case fun.(changes) do
        {:ok, result} -> apply(repo, operation, [result, opts])
